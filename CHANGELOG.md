...
- [bugfix] [hooks] **Process exit MinHook hooks wired** - `ExitProcess` and `TerminateProcess` are now hooked when API hooks install (unless suppressed), so the exit handler runs on those paths. Uninstall only disables those two detours instead of all MinHook hooks.
- [cleanup] **Auto Windows HDR success log** - When Windows HDR is turned on automatically, the log line now includes the display’s extended device id so you can see which monitor was affected.
...
- [new feature] [hooks] [experimental] **Vulkan fullscreen-exclusive acquire tracking** - Added Vulkan loader detours for `vkAcquireFullScreenExclusiveModeEXT` so calls are intercepted for diagnostics while preserving native behavior (no forced success spoofing).
  Details: `vulkan_loader_hooks` hooks both `vkGetInstanceProcAddr` and `vkGetDeviceProcAddr` resolution paths for the extension function, and tracks call counts for debug diagnostics.
...
- Details: With **`DC_NO_MODULES`**, CMake no longer compiles **`modules/audio`**, **`modules/controller`**, **`modules/example_dummy`**, or **`proxy_dll/*.cpp`** (rename-proxy forwarders); **`modules/module_registry.cpp`** stays in the target (empty registry when **`DC_INTERNAL_MODULES`** is off). **`InstallRealDXGIMinHookHooks`** is an inline no-op in **`dxgi_proxy_init.hpp`** in that configuration. **`version`** (**`version.lib`**) is linked for **`GetFileVersionInfo*`** / **`VerQueryValueW`** (**`main_entry`** ReShade detection), which **`version_proxy.cpp`** no longer pulls in. Supersedes the v0.13.85 note that module **`.obj`** files were still linked for **`DC_NO_MODULES`**.
...
- [ui] **Black curtain terminology** - Main tab checkboxes and Hotkeys tab no longer use “ADHD” in user-facing text. **Black curtain (game display)** replaces “ADHD on game display”; **Black curtain (other displays)** replaces “ADHD Multi-Monitor Mode.” The default hotkey row and input-remapping notifications use the same names. The background window title is now “Display Commander Black Curtain.” Config keys (`adhd_*`, `HotkeyAdhdToggle`, etc.) are unchanged for compatibility.
  Details: `main_new_tab.cpp`, `hotkeys_tab.cpp`, `input_remapping.cpp`, `adhd_multi_monitor.hpp`; `README.md`.
- [ui] [settings] **DXGI Control optional Main tab panel** - The **Misc** block under **VSync & Tearing** (max frame latency, buffer count with current count / flip warning) and the separate **DXGI** subsection (**Force Flip Discard upgrade**) are no longer in **Display Settings**. Enable **Show DXGI Control** under **Advanced Settings** → **Main tab optional panels** (D3D10/11/12 only). Panel id `dxgi_control` (default after **DLSS Control**). No-ReShade DXGI path: max frame latency and buffer count moved here from the inline VSync row.
  Details: `show_main_tab_dxgi_control`, `MainTabOptionalSectionKind::DxgiControl`, `DrawMainTabOptionalPanelDxgiControl`, `DrawDxgiControl_SwapchainTweaks` in `main_new_tab.cpp`; `main_tab_settings.*`.
- [ui] [settings] **DLSS Control optional Main tab panel** - The former **DLSS Information** block is no longer inside **Display Settings**. Enable it under **Advanced Settings** → **Main tab optional panels** → **Show DLSS Control**. It appears with the other optional panels (after **Advanced Settings**) as a **DLSS Control** header; panel order id `dlss_control` (default between **NVIDIA Control** and **DC folders**). When the panel is expanded but no DLSS/NGX activity is detected yet, a short message is shown instead of building the full summary.
  Details: `show_main_tab_dlss_control`, `MainTabOptionalSectionKind::DlssControl`, `DrawMainTabOptionalPanelDlssControl` in `main_new_tab.cpp`; `main_tab_settings.*`.
- [ui] **Main tab optional panels after Advanced Settings** - Sections enabled under **Advanced Settings** → **Main tab optional panels** (for example **Audio Control**, **DC folders**, **NVIDIA Control**) now render **below** the **Advanced Settings** collapsible block instead of between **Resolution Control** and **Window Control**, so the main flow stays Display → Resolution → Window → Performance overlay → Advanced → optional panels.
  Details: `DrawMainNewTab` / `DrawMainTabOptionalPanelsInOrder` order in `main_new_tab.cpp`; tooltip text in `DrawMainTabOptionalPanelsAdvancedSettingsUi`.
...
- [bugfix] [hooks] [compatibility] **FPS limiter / FG detection in Wuthering Waves (post–v0.13.12)** - Frame generation was treated as off due to changes in **v0.13.12**.
  Details: `GetDLSSGSummary` and `GetDLSSGSummaryLite` in `globals.cpp`; performance overlay FG line in `main_new_tab.cpp`.
...
- [ui] [settings] **Main tab: license / third-party notices with versioned acceptance** - The embedded **LICENSE** and **THIRD_PARTY_NOTICES** readers (RCDATA) moved to the **bottom of the Main** tab (standalone UI included). Bundled **license presentation version** starts at **1001** (`res/legal_license_version.hpp`); config key **`license_acknowledged_version`** stores the highest version the user accepted. When the bundled version is higher than the stored value, **Accept** and **Disagree** are shown (Accept saves; **Disagree** exits the process). The separate **Help** tab was removed. Details: `DrawMainTabLegalSection` in `help_tab.cpp`, `main_tab_settings.*`, `main_new_tab.cpp`, `new_ui_tabs.cpp`.
- [docs] **THIRD_PARTY_NOTICES and README license links** - Added `THIRD_PARTY_NOTICES.txt` listing external dependencies and licenses from `external/`, linked from `README.md` alongside `LICENSE`. Details: `.gitignore` exception for `THIRD_PARTY_NOTICES.txt`, `README.md` License section.
- [cleanup] **CMake: Windows SDK UM lib path for Ninja + MSVC** - Some Ninja builds failed with LNK1104 (cannot open `setupapi.lib`) when the linker had no Windows SDK `Lib/.../um/<arch>` directory on the LIBPATH. The addon CMake now derives that folder from `CMAKE_MT` and adds it via `target_link_directories` on the shared options interface so `setupapi` / `tdh` resolve without relying on a pre-set `LIB` environment. Details: `src/addons/display_commander/CMakeLists.txt`.
- [cleanup] **Optional local Git pre-push hook** - Developers can run `scripts/install_git_hooks.sh` once to install a `pre-push` hook that blocks pushes when `git ls-files -ic --exclude-standard` is non-empty (tracked paths that match ignore rules). Not used in CI. Details: `scripts/git-hooks/pre-push`, `scripts/install_git_hooks.sh`.
- [ui] **Main tab Target Display picker layout** - The display dropdown is centered in the Display Settings area and uses a flat style (no filled frame behind the preview), similar in spirit to the centered folder buttons in DC folders. Details: `DrawDisplaySettings_DisplayAndTarget` in `main_new_tab.cpp`.
- [hooks] [ui] **DLSS preset override now shows runtime hook values** - The Swapchain tab now shows the actual SR/RR preset values captured from NGX parameter hooks (plus runtime FG mode) so you can see what the game/NGX is currently using, not only what is configured in the override dropdowns. Details: `GetDLSSGSummary()` adds runtime SR/RR preset extraction from `g_ngx_parameters`; `DrawDLSSPresetOverrideContent` displays `Runtime SR Preset`, `Runtime RR Preset`, and `Runtime FG Mode (hook)`.
...
- [cleanup] [removal] **Removed unnecessary WGI code** - Unused Windows Gaming Input logic was removed to keep input hooking simpler and easier to maintain without changing the intended suppression behavior. Details: cleaned up `hooks/input/windows_gaming_input_hooks.cpp`.
...
## v0.12.570
- [ui] **Main tab backbuffer from runtime** - The Main tab Display Settings "Render resolution" line and the Brightness/AutoHDR 8-bit backbuffer warning now use the ReShade runtime passed into the overlay when available: backbuffer size from `effect_runtime::get_screenshot_width_and_height`, format (and bit depth) from `get_back_buffer(0)` + `get_resource_desc`. When no runtime is provided (e.g. standalone UI) or runtime queries fail, the UI falls back to `g_game_render_width`/`g_game_render_height` and `g_last_swapchain_desc_post` as before. Color space is not exposed on the effect_runtime API (it lives on the swapchain), so it is not shown in this section. Details: DrawMainNewTab and DrawDisplaySettings take an optional `effect_runtime*`; DrawDisplaySettings_DisplayAndTarget uses runtime for size/format; 8-bit warning uses runtime for format when non-null; new_ui_tabs.cpp passes runtime into DrawMainNewTab.
- [bugfix] [ui] **ReShade runtime list fallback** - The runtime list (used by the Main tab runtime selector and the Experimental → Runtimes tab) could stay empty when `init_effect_runtime` was missed (e.g. addon load order, or present path running before init). The overlay draw callback now ensures the current runtime is added to the list when the overlay is drawn, so the selector and Runtimes tab show at least the current runtime. Details: OnRegisterOverlayDisplayCommander calls AddReShadeRuntime(runtime); AddReShadeRuntime already deduplicates.
- [ui] [experimental] **Runtimes debug sub-tab** - Added a **Runtimes** sub-tab under Experimental (Debug) that lists all enumerated ReShade runtimes with per-runtime info: HWND, API (D3D9/D3D11/D3D12/OpenGL/Vulkan), back buffer size (width x height), back buffer count and current index, back buffer format (e.g. R8G8B8A8_UNORM, R10G10B10A2_UNORM), and color space (N/A — not exposed on effect_runtime). Useful for debugging multi-swapchain games. Details: experimental_tab.hpp DrawRuntimesDebugSubTab; experimental_tab.cpp EnumerateReShadeRuntimes callback, RuntimeSnapshot, FormatToRuntimeFormatString.
...
- [bugfix] [ui] **Standalone settings (no ReShade): Notes tab and tab visibility** - The standalone settings window (Display Commander -s or Settings (No ReShade)) now shows the **Notes** tab when "Show Notes Tab" or "Show All Tabs" is enabled. It also respects **Show All Tabs** and **Show &lt;tab&gt; Tab** settings: advanced tabs (Advanced, Controller, Performance, Performance Overlay, Vulkan, ReShade, Notes, Debug, NVIDIA Profile) are only shown when "Show All Tabs" is on or the corresponding individual checkbox is on. Main, Games, and Hotkeys remain always visible. Details: cli_standalone_ui.cpp RunStandaloneSettingsUI uses settings::g_mainTabSettings.advanced_settings_enabled and per-tab show_*_tab; added Notes tab and notes_tab.hpp include.
- [new feature] [ui] [settings] **Notes tab** - Added an optional Notes tab (hidden by default) where you can write plain-text notes about the current game. Notes are stored in a separate file per game: `%LocalAppData%\Programs\Display_Commander\Games\<GAME_NAME>\notes.txt`. Game name is derived from the game folder (e.g. `D:\...\SAMURAI MAIDEN` → `SAMURAI MAIDEN`). Enable the tab via Main → Show Individual Tabs → "Show Notes Tab", or when "Show All Tabs" is on. Save is automatic when the input loses focus; a Save button is also available. Details: utils/general_utils GetGameNotesFilePath; ui/new_ui/notes_tab.hpp, notes_tab.cpp; main_tab_settings show_notes_tab; InputTextMultiline in imgui wrapper.

## v0.12.567
- [ui] **Native Reflex (DXGI) status on same line as Reflex combo** - When the game uses D3D11/D3D12 and NvAPI Reflex, the addon now shows compact stats on the same line as the "Reflex" combo: NvAPI_D3D_Sleep count, NvAPI_D3D_SetLatencyMarker count, last marker (by type) with g_global_frame_id, and **Status: OK / FAIL**. OK when all 6 markers and NvAPI_D3D_Sleep were seen within the last 50 frames (g_global_frame_id). Per marker type (0..5) we store the last g_global_frame_id when we received that marker; we also store g_nvapi_d3d_last_sleep_global_frame_id in the Sleep detour. Same layout as the NvLL (Vulkan) status; shown when device is D3D11/D3D12 and Reflex is available. Details: DrawDxgiNativeReflexStatusOnSameLine, kDxgiNativeReflexStatusFrameWindow; g_nvapi_d3d_last_sleep_global_frame_id in globals; set in NvAPI_D3D_Sleep_Detour.

## v0.12.566
- [ui] **Native Reflex (NvLL VK) status on same line as Reflex combo** - When the game uses NvLowLatencyVk.dll (Vulkan native Reflex), the addon now shows compact stats on the same line as the "Reflex" combo: Sleep count, total Markers count, and last marker type + frame ID. Data comes only from NvLL_VK_Sleep_Detour and NvLL_VK_SetLatencyMarker_Detour, so you can see whether the game's native Reflex implementation is correct (calling Sleep and sending markers). Tooltip lists per-marker-type counts (SIMULATION_START, PRESENT_START, etc.). Shown in both OnPresent and Reflex FPS limiter sections when NvLL hooks are installed. Details: DrawNvllNativeReflexStatusOnSameLine in main_new_tab.cpp; nvlowlatencyvk_hooks already exposed GetNvllVkHookCallCounts, GetNvLowLatencyVkMarkerCountsByType, GetNvLowLatencyVkLastMarkerState.
...
## v0.12.554
- [ui] **UI tweaks** - Minor UI improvements and polish in the main tab and related screens.
...
## v0.12.551
- [hooks] **Private user32 copy for GetAsyncKeyState_Direct** - A private copy of user32.dll (e.g. `user32_dc64.dll`) is created under `%LocalAppData%\Programs\Display_Commander\dlls\` (same folder as dbghelp_dc64 / future private XInput copies). When Windows message hooks are installed, this copy is loaded and **GetAsyncKeyState_Direct** calls **GetAsyncKeyState** from the private copy instead of the process user32, so the "direct" path always uses the real API. The game's **GetAsyncKeyState** remains hooked in the process user32. If the private copy is unavailable, GetAsyncKeyState_Direct falls back to the trampoline or the linked GetAsyncKeyState. Details: hooks/user32_private_loader.hpp, hooks/user32_private_loader.cpp; InstallWindowsMessageHooks calls LoadUser32PrivateCopy; UninstallWindowsMessageHooks calls UnloadUser32PrivateCopy.
- [cleanup] [hooks] **Timeslowdown hooks: table-driven install/uninstall** - The 10 timer hooks (8 kernel32: QueryPerformanceCounter/Frequency, GetTickCount/64, GetSystemTime/AsFileTime/PreciseAsFileTime, GetLocalTime; 1 winmm: timeGetTime; 1 ntdll: NtQuerySystemTime) now use HookEntry tables and InstallHooksFromModule (kernel32, winmm) and InstallHooksFromResolver (ntdll). Uninstall no longer uses MH_DisableHook(MH_ALL_HOOKS); it uses RemoveHooksFromModule and RemoveHooksByOriginalTable so only timeslowdown hooks are removed. Details: timeslowdown_hooks.cpp; docs/timeslowdown_hooks_install_migration_plan.md.
- [settings] **global_overrides.toml replaces global_settings.toml** - The file is renamed to `global_overrides.toml` and works like `default_settings.toml` (same format: `[DisplayCommander]` and keys that match config), but values in it **override** the game's DisplayCommander.toml even when the game config already has the key. Keys in the file now match what they override: `auto_reshade_config_backup` (global "Auto-enable ReShade config backup") and `SuppressWgiEnabled` (global "Enable Windows Gaming Input suppression globally"). Old `global_settings.toml` is migrated once (AutoEnableReshadeConfigBackup → auto_reshade_config_backup, SuppressWgiGlobally → SuppressWgiEnabled). Details: config/global_overrides_file.hpp/cpp; display_commander_config Get checks override first then game config; Set goes to game config; Advanced tab uses OverrideBoolSetting for the two global overrides.
- [ui] **CheckboxSetting overload for OverrideBoolSetting** - Advanced tab global-override checkboxes (Auto-enable ReShade config backup, Enable WGI suppression globally) now use a dedicated `CheckboxSetting(OverrideBoolSetting&, ...)` overload so they build and behave correctly with global_overrides.toml. Details: settings_wrapper.hpp/cpp.

## v0.12.550
- [new feature] [settings] **default_settings.toml in AppData** - You can set your preferred Display Commander defaults in `%LocalAppData%\Programs\Display_Commander\default_settings.toml`. When a game's DisplayCommander.toml does not contain a setting, the addon uses the value from this file (after embedded per-game overrides). If the file does not exist, it is created with instructions and commented examples. Main tab DC folders: added "Default settings" button to open the file in the default editor. Details: config/default_settings_file.hpp/cpp; get_config_value_or_default uses it after embedded game_default_overrides; main_new_tab.cpp DC folders button row.
- [cleanup] **Shared ParseTomlLine** - Moved TOML key=value line parsing into config/toml_line_parser.hpp; chords_file, default_settings_file, global_settings_file, and hotkeys_file now use the shared helper. Details: toml_line_parser.hpp, chords_file.cpp, global_settings_file.cpp, hotkeys_file.cpp.
...
## v0.12.535 (reverted to)
- **Reverted to v0.12.535** - Tree reverted to this version; later work (e.g. early logger init in DllMain) reverted.
- [cleanup] [hooks] **CALL_GUARD_NO_TS(); for _Detour functions that lacked a call guard** - Added `CALL_GUARD_NO_TS();` to all detours that were missing it so crash/exit reports can attribute in-progress calls: hid_suppression_hooks.cpp (`ReadFile_Detour`, `ReadFileEx_Detour`, `ReadFileScatter_Detour`, `HidD_GetInputReport_Detour`, `HidD_GetAttributes_Detour`, `CreateFileA_Detour`, `CreateFileW_Detour`); loadlibrary_hooks.cpp (`GetModuleHandleW_Detour`, `GetModuleHandleA_Detour`, `GetModuleHandleExW_Detour`, `GetModuleHandleExA_Detour`, `GetProcAddress_Detour`); api_hooks.cpp `GetWindowPlacement_Detour`; nvapi_hooks.cpp `NvAPI_D3D_SetLatencyMarker_Detour` (guard moved to function start). Details: hid_suppression_hooks.cpp, loadlibrary_hooks.cpp, api_hooks.cpp, nvapi_hooks.cpp.
- [cleanup] **CALL_GUARD_NO_TS(); for _direct / _Direct helpers** - Applied `CALL_GUARD_NO_TS();` to all `_direct`/`_Direct` helpers that lacked a call guard: api_hooks.cpp (`IsIconic_direct`, `IsWindowVisible_direct`, `GetForegroundWindow_Direct`, `SetWindowPos_Direct`, `CreateWindowW_Direct`, `SetCursor_Direct`, `ShowCursor_Direct`, `AddVectoredExceptionHandler_Direct`, `CreateDXGIFactory1_Direct`); windows_message_hooks.cpp (`ClipCursor_Direct`, `GetAsyncKeyState_Direct`); display_settings_hooks.cpp (`ShowWindow_Direct`); loadlibrary_hooks.cpp (`LoadLibraryW_Direct`); nvapi_hooks.cpp (`NvAPI_D3D_SetSleepMode_Direct`, `NvAPI_D3D_Sleep_Direct`, `NvAPI_D3D_SetLatencyMarker_Direct`, `NvAPI_D3D_GetLatency_Direct`, `NvAPI_D3D_GetSleepStatus_Direct`); hid_suppression_hooks.cpp (`HidD_GetInputReport_Direct`). Crash/exit reports can now attribute in-progress calls at these sites. Details: utils/detour_call_tracker.hpp; added detour_call_tracker include in hid_suppression_hooks.cpp.
...
- **Known DLLs marked with * in static imports log** - The "[OnModuleLoaded] … static imports: …" line now appends an asterisk (*) to each imported DLL name that is in the Windows Known DLLs list (e.g. kernel32.dll*, user32.dll*). Those DLLs are always loaded from the system directory, never from the application folder. The list is hardcoded from HKLM\\…\\KnownDLLs. Details: utils/pe_static_imports.cpp GetKnownDllNamesLowercase, GetStaticImportDllNamesSingleLineImpl.
...
---
## v0.12.441
- [cleanup] [compatibility] **Vulkan-1 proxy: full proxy from official ABI (734 APIs)** - The vulkan-1.dll proxy is now generated from the Khronos Vulkan headers and covers **734 Vulkan entry points** (vulkan_core.h + vulkan_win32.h). Each stub uses the correct ABI (real types, VKAPI_CALL), includes Windows.h before Win32 types so HANDLE/PFN resolve, and forwards to the system vulkan-1.dll. Array parameters (e.g. blendConstants[4]) are forwarded by name correctly. Regenerate with `python scripts/gen_vulkan_proxy_from_abi.py --win32 --update-exports-def src/addons/display_commander/proxy_dll/exports.def` after header updates. Details: vulkan-1_proxy.cpp, vulkan_proxy_abi.hpp, exports.def, scripts/gen_vulkan_proxy_abi.py, scripts/gen_vulkan_proxy_from_abi.py, docs/vulkan_proxy_audit_plan.md.

---
## v0.12.440
- [bugfix] [compatibility] **Vulkan-1 proxy: return types aligned with official API** - The vulkan-1.dll proxy had 45 return-type mismatches versus the Khronos Vulkan headers. All are fixed: functions that return VkResult (e.g. vkFreeDescriptorSets, vkResetEvent, vkSetEvent, vkUnmapMemory2) now return int; vkGetInstanceProcAddr and vkGetDeviceProcAddr return LPVOID so function pointers are not truncated on 64-bit; all void-returning query/update functions (vkGet* memory/device/physical device, vkUpdateDescriptorSets, vkResetQueryPool, etc.) now return void. Ensures correct ABI and prevents callers from misreading return values. Details: vulkan-1_proxy.cpp.
- [cleanup] **Vulkan proxy: doc comparison scripts and audit plan** - Added scripts to compare the vulkan-1 proxy against official Khronos headers and to extract the API list from headers. `scripts/compare_vulkan_proxy_to_doc.py` reports missing/extra APIs, parameter count and return type mismatches. `scripts/extract_vulkan_api_from_headers.py` generates `scripts/specs/vulkan-1_official_api_list.txt`. `docs/vulkan_proxy_audit_plan.md` describes the audit process; `docs/vulkan_proxy_vs_doc_report.txt` is the latest comparison output. Details: no spec file used for comparison; doc is vulkan_core.h + vulkan_win32.h.

---
## v0.12.439
- [bugfix] **PCLStats for injected reflex: initialize when setting is on** - With "Inject Reflex" and "PCL stats for injected reflex" enabled, PCLStats was never initialized (tooltip showed "PCLStats initialized: no" even though PCLSTATS_MARKER was called). Initialization now uses only the "PCL stats for injected reflex" setting instead of the full PCLStatsReportingAllowed() check, so the ETW provider is registered and reporting works. Details: reflex_provider.cpp EnsurePCLStatsInitialized() condition.

---
## v0.12.438
- [hooks] **Vulkan loader: hook from exports only, no vkGetDeviceProcAddr** - Vulkan loader hooks are now installed by hooking vulkan-1.dll exports directly: vkGetInstanceProcAddr, vkCreateDevice, vkCreateSwapchainKHR, vkQueuePresentKHR, vkBeginCommandBuffer, vkSetLatencyMarkerNV. We no longer hook or use vkGetDeviceProcAddr to intercept device procs; all targets come from GetProcAddress(module, name). Missing exports (e.g. vkSetLatencyMarkerNV on some loaders) are skipped so hooking is correct and verifiable. Details: vulkan_loader_hooks.cpp kVulkanLoaderHooks extended to 6 entries; vkGetDeviceProcAddr_Detour removed; install loop uses GetProcAddress + CreateAndEnableHook, skips null; RollbackVulkanLoaderHooks() iterates all entries.
- [removal] [hooks] **Vulkan: remove Reflex injection, keep loader hooks for logging/stats only** - Removed all Vulkan Reflex injection to reduce crash risk (e.g. NVSDK_NGX_VULKAN_Shutdown in RDR2). We no longer add VK_NV_low_latency2 extensions in vkCreateDevice, do not add VkSwapchainLatencyCreateInfoNV in vkCreateSwapchainKHR, and do not inject present ID, markers, or vkLatencySleepNV in vkQueuePresentKHR or vkBeginCommandBuffer. Vulkan loader hooks (vkGetInstanceProcAddr, vkGetDeviceProcAddr, vkCreateDevice, vkSetLatencyMarkerNV, vkQueuePresentKHR, vkCreateSwapchainKHR, vkBeginCommandBuffer) remain installed for logging, call counts, extension capture, and FPS limiter only. VulkanInjectedReflexDebugState fields are kept for UI compatibility but are always zero/false. Details: vulkan_loader_hooks.cpp remove append_extensions, ResolveInjectedReflexProcs, swapchain latency and semaphore creation, inject blocks in vkQueuePresentKHR and vkBeginCommandBuffer; vulkan_loader_hooks.hpp comment updates.

---
## v0.12.437
- [ui] [settings] **Brightness/AutoHDR: split into two checkboxes** - The single "Enable Brightness, AutoHDR and ReShade paths" checkbox is now two: "Add DC Shaders/Textures to ReShade paths" (controls adding/removing DC's folder from ReShade's EffectSearchPaths and TextureSearchPaths) and "Load DC's shaders (Brightness, AutoHDR)" (controls whether Brightness, AutoHDR and related effects are applied). You can add paths without enabling DC effects, or enable DC effects with paths added elsewhere. Details: main_tab_settings add_dc_to_reshade_shader_paths; main_entry.cpp OverrideReShadeSettings_AddDisplayCommanderPaths/OverrideReShadeSettings use add_dc_to_reshade_shader_paths; main_new_tab.cpp two checkboxes with separate tooltips.

---
## v0.12.436
- [cleanup] **display_settings_hooks moved to hooks/system/** - display_settings_hooks.cpp and .hpp are now under `hooks/system/` with the other system-level hooks. All consumer includes and docs updated.
...
## v0.12.426
- [new feature] [hooks] [ui] **Vulkan injected Reflex (VK_NV_low_latency2)** - Added optional injection of NVIDIA Reflex for Vulkan games that do not use native Reflex (NvLowLatencyVk.dll). When "Enable injected Reflex for Vulkan" is on in the Vulkan tab (default off), the addon appends VK_NV_low_latency2 extensions in vkCreateDevice, adds swapchain latency mode in vkCreateSwapchainKHR, injects present ID and latency markers in vkQueuePresentKHR and vkBeginCommandBuffer, and calls vkLatencySleepNV with a timeline semaphore to reduce input latency. Injection is skipped when NvLowLatencyVk.dll is loaded. A new "Injected Reflex (debug)" section in the Vulkan tab shows metrics: setting/hooks/NvLL status, injecting flag, present ID, markers injected, sleep calls, swapchain latency creates, device/swapchain/semaphore and procs resolved. Details: spec in spec_vulkan_injected_reflex.md; main_tab_settings vulkan_injected_reflex_enabled (default false); vulkan_loader_hooks.cpp vkCreateSwapchainKHR/vkBeginCommandBuffer detours and vkQueuePresentKHR injection path; vulkan_loader_hooks.hpp VulkanInjectedReflexDebugState, GetVulkanInjectedReflexDebugState; vulkan_tab.cpp checkbox and debug header.

## v0.12.425
- [new feature] [hooks] **Vulkan FPS limiter via vkQueuePresentKHR** - When Vulkan loader hooks are enabled, the addon now intercepts `vkQueuePresentKHR` (when the game looks it up via `vkGetDeviceProcAddr`) and runs the same FPS limiter flow as the DXGI Present detour: ChooseFpsLimiter, OnPresentFlags2, RecordNativeFrameTime, RecordFrameTime, then call the real present, then HandlePresentAfter. Vulkan games without Reflex markers can use the FPS limiter from this present path. A new FPS limiter call site `vk_queue_present_khr` was added; it is preferred over DXGI/ReShade when active (after Reflex paths). Details: vulkan_loader_hooks.cpp vkQueuePresentKHR_Detour and vkGetDeviceProcAddr interception; globals.hpp/cpp FpsLimiterCallSite::vk_queue_present_khr, priority order, FpsLimiterSiteName, GetChosenFrameTimeLocation.

## v0.12.424
- [hooks] **Vulkan: hook vkCreateDevice export directly** - vkCreateDevice is now hooked on the vulkan-1.dll export as well as via vkGetInstanceProcAddr. Games that resolve vkCreateDevice with GetProcAddress(module, "vkCreateDevice") instead of vkGetInstanceProcAddr(instance, "vkCreateDevice") will now be intercepted, so call counts and Reflex extension injection work for both code paths. Details: vulkan_loader_hooks.cpp kVulkanLoaderHooks + vkCreateDevice entry; vkGetInstanceProcAddr still returns detour for "vkCreateDevice".
- [settings] [hooks] **Vulkan loader hooks enabled by default** - The "Enable vulkan-1 loader hooks" setting now defaults to true so Vulkan loader hooks install automatically for Vulkan games. Users who previously saw "VulkanLoader: hooks disabled by setting" with no config change will now get hooks enabled by default. Details: main_tab_settings.cpp vulkan_vk_loader_hooks_enabled default true.
- [new feature] [compatibility] **vulkan-1.dll proxy** - Display Commander can be used as a vulkan-1.dll proxy (rename/copy the addon DLL to vulkan-1.dll in the game directory). All Vulkan loader exports (262 functions from Wine’s vulkan-1.spec) are forwarded to the real system vulkan-1.dll. Enables loading the addon into Vulkan games the same way as with d3d9, d3d11, or dinput8. Proxy exports and C++ stub are generated by `scripts/gen_proxy_from_spec.py` from `scripts/specs/vulkan-1.spec`. Details: proxy_dll/vulkan-1_proxy.cpp, exports.def vulkan-1 block; main_entry.cpp proxy_dlls vulkan-1 entry; gen_proxy_from_spec.py vulkan-1 loader and return-type handling.

## v0.12.423
- [cleanup] **Vulkan loader hooks: rename wrappers to _Detour** - In vulkan_loader_hooks.cpp, `vkCreateDevice_Wrapper` and `VkSetLatencyMarkerNV_Wrapper` are renamed to `vkCreateDevice_Detour` and `vkSetLatencyMarkerNV_Detour` so naming matches other hook files (e.g. api_hooks.cpp). Log and comment wording updated from "wrapper" to "detour". Behavior unchanged. Details: vulkan_loader_hooks.cpp.
- [cleanup] **Vulkan loader hooks: table-driven install** - Vulkan loader hook installation now uses a single loop over a `kVulkanLoaderHooks` table (name, detour, original) and `CreateAndEnableHookFromModule`, with `RollbackVulkanLoaderHooks` on failure. Same pattern as other hook installers; adding a new Vulkan loader hook is a single table entry. Details: vulkan_loader_hooks.cpp VulkanLoaderHookEntry, InstallVulkanLoaderHooksImpl.
- [cleanup] **Vulkan loader: simplify detours, remove dummies** - Vulkan loader detours now simply return error or null when the original function is not available; all dummy implementations (Dummy_vkSetLatencySleepModeNV, Dummy_vkLatencySleepNV, Dummy_vkSetLatencyMarkerNV, Dummy_vkGetLatencyTimingsNV) and `ReturnDummyIfNull` / `GetVulkanLoaderDummyCallCounts` are removed. The Vulkan tab no longer shows "Dummy procs (loader returned null)". Details: vulkan_loader_hooks.cpp, vulkan_loader_hooks.hpp, vulkan_tab.cpp.
- [ui] **Vulkan tab: reason when no extension data** - When the "Enabled extensions" list is empty, the Vulkan tab now shows a specific reason: enable vulkan-1 loader hooks, vkCreateDevice has not been called yet, or vkCreateDevice was called but no extensions were reported. Uses `HasVulkanCreateDeviceBeenCalled()`. Details: vulkan_tab.cpp; vulkan_loader_hooks g_vkCreateDevice_detour_ever_called.
- [ui] **Vulkan tab: call counts per loader function** - The Vulkan tab Debug section now shows call counts for each Vulkan loader detour: vkGetInstanceProcAddr, vkGetDeviceProcAddr, vkCreateDevice, and vkSetLatencyMarkerNV. Helps see how often the game uses these paths. Details: GetVulkanLoaderCallCounts; vulkan_loader_hooks.cpp g_calls_*; vulkan_tab.cpp.
- [new feature] [hooks] **Vulkan loader hook suppression** - Vulkan loader hooks (vkGetInstanceProcAddr, vkGetDeviceProcAddr on vulkan-1.dll) can be suppressed like other hook types. When suppressed, the addon does not install these hooks. Use Debug → Hooks and enable "Suppressed" for "Vulkan Loader", or set `VulkanLoaderHooks = true` under `[DisplayCommander.HookSuppression]` in DisplayCommander.toml. Takes effect on next vulkan-1.dll load or when the loader is already loaded and you enable the Vulkan checkbox. Details: HookType::VULKAN_LOADER; suppress_vulkan_loader_hooks / vulkan_loader_hooks_installed; InstallVulkanLoaderHooksImpl checks ShouldSuppressHook and calls MarkHookInstalled on success.
- [ui] [removal] **Vulkan: always append Reflex extensions, remove checkbox** - Reflex extensions (VK_NV_low_latency2, VK_KHR_present_id, VK_KHR_timeline_semaphore) are now always appended in vkCreateDevice when vulkan-1 loader hooks are active; the "Append Reflex extensions in vkCreateDevice" checkbox is removed from the Vulkan tab. Simplifies the UI; behavior is the same as previously with the option enabled. Details: vulkan_loader_hooks.cpp append_extensions = true; main_tab_settings remove vulkan_append_reflex_extensions; vulkan_tab.cpp remove checkbox.
...
## v0.12.379
- **Steam achievement notifications: run only on background thread** - The logic that detects an achievement unlock and updates the "bump" state (including the Steam API call to resolve the last-unlocked achievement name) no longer runs on the main/render thread. It now runs only in the ContinuousMonitoring thread inside `RefreshSteamAchievementCacheFromBackground()`. The overlay (`OnSteamAchievementOverlay`) on the main thread only reads precomputed bump state and draws; it no longer calls `SetSteamAchievementBumpFromUnlock` or `ClearSteamAchievementLastUnlocked`. This avoids blocking the game’s render thread on Steam API. Details: steam_achievement_cache.cpp (bump update and clear in RefreshSteamAchievementCacheFromBackground); main_entry.cpp OnSteamAchievementOverlay (removed bump/clear; comment that main thread only reads and draws); steam_achievement_cache.hpp (SetSteamAchievementBumpFromUnlock thread contract).
- **Steam achievement API naming: NonBlocking / Blocking suffixes** - Cache and overlay APIs now use consistent suffixes: `GetSteamAchievementCountCachedNonBlocking()` (overlay/UI); blocking Steam helpers `GetSteamAchievementCountBlocking`, `GetSteamAchievementListBlocking`, `GetSteamAchievementExportsDebugBlocking`, `GetLastUnlockedAchievementInfoBlocking`; overlay-only bump APIs `IsSteamAchievementBumpActiveNonBlocking`, `GetSteamAchievementBumpDisplayNonBlocking`, `GetSteamAchievementBumpTextNonBlocking`. Makes it explicit which functions are safe to call from the main thread. Details: steam_achievement_cache.hpp/cpp, steam_achievements.hpp/cpp, main_entry.cpp, advanced_tab.cpp, continuous_monitoring.cpp.
...
## v0.12.359
- **Cleanup: remove unused MainTabSettings::GetAllSettings** - The method was never called; MainTabSettings::LoadSettings() uses all_settings_ directly via LoadTabSettingsWithSmartLogging. Removed declaration from main_tab_settings.hpp and implementation from main_tab_settings.cpp. Details: settings/main_tab_settings.hpp, settings/main_tab_settings.cpp.
- **Cleanup: remove more dead commented code** - Removed commented "already wrapped" / QuerySwapChainWrapper block in CreateSwapChainWrapper (dxgi_factory_wrapper.cpp). Replaced obsolete commented guard in swapchain_events presentBefore device-name block with a short clarifying comment. Details: hooks/dxgi_factory_wrapper.cpp, swapchain_events.cpp.
- **Cleanup: remove dead commented code and unused ASPECT_OPTIONS export** - Removed commented `return false` lines in Streamline hook setup (streamline_hooks.cpp). Removed redundant commented ClipCursor input-block block and SetCapture block in windows_message_hooks.cpp. Removed commented init/QuerySwapChainWrapper block in swapchain_events.cpp and condensed D3D9Ex return comment. Removed commented else/brace and ReleaseCapture() in continuous_monitoring.cpp. Made ASPECT_OPTIONS file-static in general_utils.cpp and removed unused extern from general_utils.hpp. Details: hooks/streamline_hooks.cpp, hooks/windows_hooks/windows_message_hooks.cpp, swapchain_events.cpp, continuous_monitoring.cpp, utils/general_utils.hpp, utils/general_utils.cpp.
- **Cleanup: remove unused IntSettingRef using** - advanced_tab_settings.hpp had `using ui::new_ui::IntSettingRef;` but no member uses IntSettingRef (the only related member enable_d3d9e_upgrade is commented out and was BoolSettingRef). Removed the unused using. Details: settings/advanced_tab_settings.hpp.
- **Cleanup: remove unused XInput widget Cleanup API** - The free function `CleanupXInputWidget()` and the class method `XInputWidget::Cleanup()` were never called (no shutdown path invokes them). Removed declaration from xinput_widget.hpp, implementation and the free function from xinput_widget.cpp. Details: widgets/xinput_widget/xinput_widget.hpp, widgets/xinput_widget/xinput_widget.cpp.
- **Cleanup: remove unused DrawDisplaySettings_VSyncAndTearing_FpsSliders** - The static function in main_new_tab.cpp was never called. The same Background FPS / background FPS limit UI is already shown in the FPS limiter section (DrawDisplaySettings_FpsLimiter). Removed the dead function. Details: ui/new_ui/main_new_tab.cpp.
- **Cleanup: remove unused settings wrapper helpers** - Four UI helpers in settings_wrapper were never called: `SliderFloatSettingRef`, `SliderIntSetting(IntSettingRef& ...)`, `ComboSettingRefWrapper`, and `CheckboxSetting(BoolSettingRef& ...)`. No settings use the Ref types. Removed declarations and implementations. Details: ui/new_ui/settings_wrapper.hpp, ui/new_ui/settings_wrapper.cpp.
- **Cleanup: remove junk file utils/Untitled** - The file contained only "sekiro.exe" and was not source code; removed as accidental/temporary file. Details: utils/Untitled deleted.
- **Cleanup: remove unused DrawDisplaySettings_FpsAndBackground** - The function was never called; its content had been moved to DrawDisplaySettings_FpsLimiter / DrawDisplaySettings_FpsLimiterAdvanced. Removed declaration from main_new_tab.hpp and implementation from main_new_tab.cpp.
- **Cleanup: remove unused TryHardLinkOrCopyFile** - The function in utils/general_utils was never called. Removed declaration and implementation. Details: utils/general_utils.hpp, utils/general_utils.cpp.
- **Cleanup: remove unused ReShade version dropdown state in standalone UI** - Three static variables in cli_standalone_ui.cpp (s_reshadeVersions, s_reshadeVersionIndex, s_reshadeVersionsInitialized) were never read or written after their declarations; the version list/selection UI that would have used them was removed or never completed. Removed the dead variables. Details: ui/cli_standalone_ui.cpp.
- **Cleanup: remove more unused standalone UI code** - Six static functions and one cache in cli_standalone_ui.cpp had no callers: GetExeProductNameUtf8, FindLargestExeInDir, WstringToUtf8, GetPidByExePath, RedactPathForDisplay, GetCachedFileSha256, and the ReshadeSha256CacheEntry struct with s_sha256Cache64/32. Removed them and the now-unused includes (file_sha256.hpp, helper_exe_filter.hpp, reshade_sha256_database.hpp). Details: ui/cli_standalone_ui.cpp.
- **Cleanup: remove unused VBlankMonitor::GetThreadPhase and vblank_monitor_integration** - GetThreadPhase() had no callers. The file vblank_monitor_integration.cpp (dxgi::fps_limiter::StartVBlankMonitoring/StopVBlankMonitoring/IsVBlankMonitoringActive) was never called; VBlank monitoring is done via LatentSyncLimiter and its embedded VBlankMonitor. Removed the getter from vblank_monitor.hpp and deleted vblank_monitor_integration.cpp. Details: latent_sync/vblank_monitor.hpp; removed latent_sync/vblank_monitor_integration.cpp; docs/project_structure.md.
- **Cleanup: remove unused logger force_auto_flush_count_** - The DisplayCommanderLogger member force_auto_flush_count_ was never written (no code ever incremented or set it); the branch in Log() that pushed a flush sentinel when it was > 0 was dead. Removed the member and the branch. Details: utils/display_commander_logger.hpp, utils/display_commander_logger.cpp.
- **Cleanup: remove unused standalone UI helpers** - Six static functions and three name arrays in cli_standalone_ui.cpp had no callers: IsReshadeApiProxyDll, CollectReShadeDllsInDir, CollectDisplayCommanderAddonsInDir, RemoveWrongArchitectureFiles, DllHasExport, ShowReshadeCoreVersionsForDir, and the arrays s_reshadeDllNames, s_reshadeApiProxyNames, s_reshadeCoreNames. Removed to reduce dead code. Details: ui/cli_standalone_ui.cpp.
- **Cleanup: remove unused LogWarnThrottled macro** - The throttled-warning macro in utils/logging.hpp was never used; LogInfoThrottled, LogErrorThrottled, and LogDebugThrottled remain. Removed the macro and its comment. Details: utils/logging.hpp.
- **Cleanup: remove unused LogInfoDirectSynchronized and FormatMessageDirectRelativeTime** - The logger member LogInfoDirectSynchronized and any free-function overload had no callers. Removed the member, the helper FormatMessageDirectRelativeTime, and the now-unused baseline_ns_ field from DisplayCommanderLogger. Details: utils/display_commander_logger.hpp, utils/display_commander_logger.cpp.
- **Cleanup: remove unused LatentSyncManager init/shutdown API** - `InitializeLatentSyncSystem` and `ShutdownLatentSyncSystem` were declared in latent_sync_manager.hpp and implemented in latent_sync_manager.cpp but never called; only `GetLatentLimiter()` is used. Removed both methods and the now-unused logging include from latent_sync_manager.cpp. Details: latent_sync/latent_sync_manager.hpp, latent_sync/latent_sync_manager.cpp.
- **Cleanup: remove unused stack_trace include from experimental_tab.cpp** - The file included utils/stack_trace.hpp but did not use GenerateStackTrace or any other symbol from it. Removed the include. Details: ui/new_ui/experimental_tab.cpp.
- **Cleanup: remove unused UninstallDX9Hooks** - The function was declared in hooks/d3d9/d3d9_hooks.hpp and implemented in d3d9_hooks.cpp but never called (no teardown path for D3D9 hooks; UninstallApiHooks does not call it). Removed declaration and implementation. Details: hooks/d3d9/d3d9_hooks.hpp, hooks/d3d9/d3d9_hooks.cpp.
- **Cleanup: remove unused UninstallNvLowLatencyVkHooks** - The function was declared in hooks/vulkan/nvlowlatencyvk_hooks.hpp and implemented in nvlowlatencyvk_hooks.cpp but never called (no teardown path for NvLowLatencyVk hooks). Removed declaration and implementation. Details: hooks/vulkan/nvlowlatencyvk_hooks.hpp, hooks/vulkan/nvlowlatencyvk_hooks.cpp.
- **Cleanup: remove unused UninstallDisplaySettingsHooks** - The function was declared in display_settings_hooks.hpp and implemented in display_settings_hooks.cpp but never called (no teardown path for display settings hooks). Removed declaration and implementation. Details: hooks/display_settings_hooks.hpp, hooks/display_settings_hooks.cpp.
- **Cleanup: remove unused XInput widget Recenter API** - Six class methods (DrawRecenterSettings, ClearRecenterData, StartRecenterRecording, StopRecenterRecording, ProcessRecenterData, ApplyRecenterCalibration) and the free function ProcessRecenterData were declared in widgets/xinput_widget/xinput_widget.hpp but had no implementations and no callers. Removed all seven dead declarations. Details: widgets/xinput_widget/xinput_widget.hpp.
- **Cleanup: remove unused UninstallVulkanLoaderHooks** - The function was declared in hooks/vulkan/vulkan_loader_hooks.hpp and implemented in vulkan_loader_hooks.cpp but never called (no teardown path for Vulkan loader hooks). Removed declaration and implementation. Details: hooks/vulkan/vulkan_loader_hooks.hpp, hooks/vulkan/vulkan_loader_hooks.cpp.
- **Cleanup: remove unused WGI CoreWindowProxy module** - The hooks/wgi/corewindow_proxy.cpp and .hpp were never referenced outside the file; no other code includes or uses CoreWindowProxy. Removed both files. Details: hooks/wgi/corewindow_proxy.cpp, hooks/wgi/corewindow_proxy.hpp.
- **Cleanup: remove unused GetReshadeInstalledVersionList** - The function in utils/reshade_load_path was never called. Removed declaration and implementation. Details: utils/reshade_load_path.hpp, utils/reshade_load_path.cpp.
- **Cleanup: remove unused GetReshadeLoadFallbackVersionInfo** - The function in utils/reshade_load_path was never called. The fallback version strings were stored but never read. Removed the getter, the two static strings, and the assignments to them; ChooseReshadeVersionResult still carries fallback info for logging. Details: utils/reshade_load_path.hpp, utils/reshade_load_path.cpp.
- **Cleanup: remove unused IsReshadeLoadDisabledByConfig and CopyCurrentReshadeToDll** - Both functions in utils/reshade_load_path had no callers. Removed declarations from reshade_load_path.hpp and implementations from reshade_load_path.cpp.
- **Cleanup: remove unused LatentSyncLimiter OnFrameBegin/OnFrameEnd** - The two empty inline hooks had no callers; only OnPresentEnd is used for present timing. Removed from latent_sync_limiter.hpp.
- **Cleanup: remove unused ADHD IsFocusDisengage API** - IsFocusDisengage was declared in adhd_simple_api.hpp and adhd_multi_monitor.hpp and implemented in adhd_simple_api.cpp but never called. Focus disengagement is always enabled with no UI control. Removed from all three files.
- **Cleanup: remove unused UninstallDDrawHooks** - The function was declared in ddraw_present_hooks.hpp and implemented in ddraw_present_hooks.cpp but never called (no teardown path for DDraw hooks). Removed declaration and implementation. Details: hooks/ddraw/ddraw_present_hooks.hpp, hooks/ddraw/ddraw_present_hooks.cpp.
- **Cleanup: remove unused IsNvngxUpdateRunning** - The function in utils/stack_trace was never called. Removed declaration, implementation, and the now-unused includes (tlhelp32.h, cstring). Details: utils/stack_trace.hpp, utils/stack_trace.cpp.
- **Cleanup: remove unused NeedsWindowAdjustment** - The function was declared in addon.hpp but had no implementation and no callers. Removed the dead declaration. Details: addon.hpp.
- **Cleanup: remove unused GetStackTraceString** - The two overloads (no-arg and CONTEXT*) in utils/stack_trace were never called; callers use GenerateStackTrace() and format frames themselves. Removed declarations and implementations. Details: utils/stack_trace.hpp, utils/stack_trace.cpp.
- **Cleanup: remove unused IsSteamAPIInitExportPresent** - The function in utils/platform_api_detector was never called; callers use IsSteamAPIModuleLoaded and IsSteamAPIExportPresent directly. Removed declaration and implementation. Details: utils/platform_api_detector.hpp, utils/platform_api_detector.cpp.
- **Cleanup: remove unused ShouldApplyWindowedForBackbuffer** - The function was declared in addon.hpp and window_management.hpp but had no implementation and no callers. Removed both declarations. Details: addon.hpp, window_management/window_management.hpp.
- **Cleanup: remove unused IsHIDSuppressionEnabled** - The function was declared in hid_suppression_hooks.hpp and implemented in hid_suppression_hooks.cpp but never called; callers use ShouldSuppressHIDInput() for the same setting or SetHIDSuppressionEnabled/AreHIDSuppressionHooksInstalled for UI. Removed declaration and implementation. Details: hooks/hid_suppression_hooks.hpp, hooks/hid_suppression_hooks.cpp.
- **Cleanup: remove unused DirectInput device-state hook API** - `HookDirectInputDeviceVTable`, `DInputDevice_GetDeviceState_Detour`, and `DInputDevice_GetDeviceData_Detour` were declared in dinput_hooks.hpp but never implemented or called; device-state hooking was planned but not implemented. Also removed the unused type alias `DirectInputCreateDevice_pfn`. Details: hooks/dinput_hooks.hpp.
- **Cleanup: remove unused includes from hid_suppression_hooks.cpp** - The file did not use symbols from `hid_hooks_install.hpp` or `hook_suppression_manager.hpp`; removed both includes. Details: hooks/hid_suppression_hooks.cpp.
- **Cleanup: remove redundant timing.hpp include from display_settings_hooks.cpp** - The file already includes `../utils.hpp`, which pulls in `utils/timing.hpp`; removed the duplicate explicit include. Details: hooks/display_settings_hooks.cpp.
- **Cleanup: remove duplicate or redundant timing/general_utils includes** - nvapi_hooks.cpp had `timing.hpp` twice (../utils/ and utils/); removed the duplicate. opengl_hooks.cpp and windows_message_hooks.cpp already include `utils.hpp` (which pulls in timing and general_utils); removed redundant explicit timing.hpp and, in windows_message_hooks.cpp, redundant general_utils.hpp. Details: hooks/nvapi_hooks.cpp, hooks/opengl_hooks.cpp, hooks/windows_hooks/windows_message_hooks.cpp.
- **Cleanup: remove redundant utils/general_utils and utils/timing includes** - globals.cpp already includes `utils.hpp` (which includes general_utils.hpp); removed redundant `utils/general_utils.hpp`. main_new_tab.cpp already includes `../../utils.hpp`; removed redundant `../../utils/general_utils.hpp` and `utils/timing.hpp`. Details: globals.cpp, ui/new_ui/main_new_tab.cpp.
- **Cleanup: remove unused get_timer_resolution_ms declaration** - The function was declared in utils/timing.hpp but had no implementation (timing.cpp has get_timer_resolution_qpc instead) and no callers. Removed the dead declaration. Details: utils/timing.hpp.
- **Cleanup: remove unused RemoveExtraReshadeApiProxyDlls and cli_detect_exe include** - The static function in cli_standalone_ui.cpp was never called; the file did not use any symbol from ui/cli_detect_exe.hpp. Removed the dead function and the redundant include. Details: ui/cli_standalone_ui.cpp.
- **Cleanup: remove redundant utils.hpp and fix includes in four hook files** - process_exit_hooks.cpp already had general_utils.hpp and timing.hpp; removed utils.hpp. hid_statistics.cpp used only algorithm/string; removed utils.hpp. corewindow_proxy.cpp used only logging; removed utils.hpp. hid_suppression_hooks.cpp needed srwlock_registry for g_hid_suppression_mutex; removed utils.hpp and added explicit utils/srwlock_registry.hpp. Details: hooks/process_exit_hooks.cpp, hooks/hid_statistics.cpp, hooks/wgi/corewindow_proxy.cpp, hooks/hid_suppression_hooks.cpp.
- **Cleanup: remove unused DDS texture dump 1D/3D APIs** - `DumpTexture1DToDDS` and `DumpTexture3DToDDS` in utils/dds_texture_dump were never called; only `DumpTexture2DToDDS` is used by the D3D11 texture dump feature. Removed the two unused declarations and implementations and the `DDSCAPS2_VOLUME` constant that was only used by the 3D path. Details: utils/dds_texture_dump.hpp, utils/dds_texture_dump.cpp.
- **Cleanup: remove duplicate timing.hpp include from dinput_hooks.cpp** - The file had `#include "../utils/timing.hpp"` twice; removed the duplicate. Details: hooks/dinput_hooks.cpp.
- **Cleanup: remove unused include from sleep_hooks.cpp** - Dropped `../utils.hpp` (file uses only general_utils for CreateAndEnableHook; no timing). Details: hooks/sleep_hooks.cpp.
- **Cleanup: remove unused IsWindowWithTitleVisible** - The function was declared in overlay_window_detector.hpp but had no implementation and no callers. Removed the dead declaration. Details: utils/overlay_window_detector.hpp.
- **Fix: restore detour_call_tracker include in addon.cpp** - addon.cpp uses CALL_GUARD(utils::get_now_ns()) and must include utils/detour_call_tracker.hpp for the macro. The include was incorrectly removed in a prior cleanup. Details: addon.cpp.
- **Cleanup: remove unused UI color constant** - TEXT_BRIGHT in res/ui_colors.hpp had no references. Removed to reduce dead code.
- **Cleanup: remove unused DetectExeForPath** - The function was declared in ui/cli_detect_exe.hpp and implemented in addon.cpp but never called (DetectExe CLI uses find_largest_exe_in_dir + detect_exe_impl). Removed declaration and implementation. Details: ui/cli_detect_exe.hpp, addon.cpp.
- **Cleanup: remove unused UI color constants from ui_colors.hpp** - Removed six unused constants: ICON_CAUTION, ICON_DANGER, ICON_INFO, ICON_NEUTRAL, ICON_MUTED, TEXT_BRIGHT. All other colors and helpers remain in use. Details: res/ui_colors.hpp.
- **Cleanup: remove unused includes from windows_gaming_input_hooks.cpp** - Dropped `../utils.hpp` (file uses only general_utils and srwlock_wrapper) and `globals.hpp` (no globals used in this file). Details: hooks/windows_gaming_input_hooks.cpp.
- **Cleanup: remove unused #include detour_call_tracker.hpp from addon.cpp** - addon.cpp did not use any symbol from detour_call_tracker; the include was redundant. Details: addon.cpp.
- **Cleanup: remove unused g_d3d9_present_hooks_installed_internal** - The atomic in the d3d9 present hooks anonymous namespace was never read or written; hook state uses g_d3d9_present_hooks_installed. Removed the dead variable. Details: hooks/d3d9/d3d9_present_hooks.cpp.
- **Cleanup: remove unused UnhookD3D9Present** - The function was declared in d3d9_present_hooks.hpp and implemented in d3d9_present_hooks.cpp but never called (no teardown path wires it). Removed declaration and implementation. Details: hooks/d3d9/d3d9_present_hooks.hpp, hooks/d3d9/d3d9_present_hooks.cpp.
- **Cleanup: remove unused ProcessStickInput declaration** - ProcessStickInput(value, deadzone, max_input, min_output) was declared in general_utils.hpp but had no implementation and no callers; callers use ProcessStickInputRadial and ProcessStickInputSquare. Removed the dead declaration. Details: utils/general_utils.hpp.
- **Cleanup: remove unused WIDTH_OPTIONS, HEIGHT_OPTIONS** - The width/height option arrays in general_utils were never referenced; only ASPECT_OPTIONS (used by GetAspectByIndex) remains. Removed from general_utils.hpp and general_utils.cpp.
- **Cleanup: remove dead monitor list (MonitorEnumProc, MonitorInfo, g_monitors)** - MonitorEnumProc was never called (all EnumDisplayMonitors call sites use lambdas). The global g_monitors was never populated or read. Removed MonitorEnumProc, struct MonitorInfo, and g_monitors from general_utils and globals. Details: utils/general_utils.hpp, utils/general_utils.cpp; globals.hpp, globals.cpp.
- **Cleanup: remove unused GetLocalDcVersion and GetDcVersionInDirectory** - GetLocalDcVersion was never called (UI uses GetDLLVersionString on paths from GetDcAddonPathInDirectory). GetDcVersionInDirectory was only used by GetLocalDcVersion. Removed both declarations from dc_load_path.hpp and implementations from dc_load_path.cpp. Details: utils/dc_load_path.hpp, utils/dc_load_path.cpp.
- **Cleanup: remove obsolete OnSetFullscreenState comments** - Removed the "FORWARD DECLARATIONS" section and OnSetFullscreenState note from swapchain_events.hpp, and the trailing duplicate comment from swapchain_events.cpp. Fullscreen prevention is already documented in main_entry where the event was unregistered. Details: swapchain_events.hpp, swapchain_events.cpp.
- **Cleanup: remove unused AutoSetColorSpace(swapchain) declaration** - The single-parameter overload was declared in swapchain_events.hpp but never defined or called; only the two-parameter AutoSetColorSpace(swapchain, dxgi_swapchain) in swapchain_events.cpp is used. Removed the dead declaration and left a short comment. Details: swapchain_events.hpp.
- **Cleanup: remove unused GetAddonDirectory** - The function was declared in general_utils.hpp and implemented in general_utils.cpp but had no callers. Removed declaration and implementation. Details: utils/general_utils.hpp, utils/general_utils.cpp.
- **Cleanup: remove unused GetSyncIntervalCoefficient** - The function in swapchain_events.cpp mapped sync interval values to FPS coefficients but was never called. Removed the dead implementation.
- **Cleanup: remove redundant OnPresentUpdateAfter2 forward declaration** - swapchain_events.hpp had both a forward and a full declaration of OnPresentUpdateAfter2; the forward was redundant. Details: swapchain_events.hpp.
- **Cleanup: remove unused LoadRealDXGIFromDllMain** - The function was declared in dxgi_proxy_init.hpp and implemented in dxgi_proxy.cpp but never called. Removed declaration and implementation. Details: proxy_dll/dxgi_proxy_init.hpp, proxy_dll/dxgi_proxy.cpp.
- **Cleanup: remove unused GetLastColorSpaceSupportForUI** - The function and the two atomics it read (`g_last_color_space_support_dxgi`, `g_last_color_space_support_result`) had no callers; the state was written in SetSwapChainColorSpace but never used. Removed declaration from swapchain_events.hpp, implementation and atomics from swapchain_events.cpp, and the two store calls in SetSwapChainColorSpace.
- **Cleanup: remove unused resolution_helpers declarations** - `GetSelectedRefreshRateRational` and `ApplyDisplaySettingsModern` were declared in resolution_helpers.hpp but had no implementation and no callers. Removed the dead declarations; `ApplyDisplaySettingsDXGI` remains and is used. Details: resolution_helpers.hpp.
- **Cleanup: remove unused dc_load_path APIs** - `GetDcSelectedVersionFromConfig`, `SetDcSelectedVersionInConfig`, `GetDcInstalledVersionList` (legacy alias), and `GetDcProxyDirectory` had no callers (migration uses config keys directly; UI uses `GetDcInstalledVersionListStable`/`Debug` and `GetDcProxyModulePath`). Removed from utils/dc_load_path.hpp and dc_load_path.cpp.
- **Cleanup: remove deprecated texture tracker duplicate 2D fields** - The five deprecated fields in `TextureTrackerStats` (texture_cache_hits, texture_cache_lookups, texture_cache_lookup_misses, texture_cache_inserts, texture_cache_total_bytes) were duplicates of texture_cache_2d and were never read by the UI (which uses texture_cache_1d/2d/3d). Removed from the struct and from TextureTrackerGetStats. Details: utils/texture_tracker.hpp, utils/texture_tracker.cpp.
- **Cleanup: remove obsolete commented hookToSwapChain in OnPresentUpdateBefore** - Removed the leftover "how to check" comment and commented-out hookToSwapChain(swapchain) call; swapchain hooking is done elsewhere (initSwapchain). Details: swapchain_events.cpp.
- **Cleanup: remove no-op QueryDxgiCompositionState and unused g_last_swapchain_ptr_unsafe** - The DXGI composition / independent flip query was already a no-op and no longer shown in UI. Removed the function, its declaration in swapchain_events.hpp, and all four call sites from the DXGI present detours. Removed the unused global `g_last_swapchain_ptr_unsafe` (store had been removed earlier; the only reader was commented-out device cleanup), the commented-out cleanup block, and related comments. Details: swapchain_events.cpp/.hpp; hooks/dxgi/dxgi_present_hooks.cpp; globals.hpp; globals.cpp.
- **Cleanup: remove unused CopyCurrentVersionToDll and CopyCurrentVersionToGlobal** - These version_check APIs had no callers. Removed declarations from version_check.hpp and implementations from version_check.cpp.
- **Cleanup: remove no-op MarkAdditionalHIDHooksInstalled** - The function was write-only (no readers). Removed the function, its declaration in hid_additional_hooks.hpp, and both call sites in hid_hooks_install.cpp.
- **Cleanup: remove unused texture tracker *2D() APIs** - Five dimension-specific 2D functions (TextureCacheLookupRecord2D, TextureCacheLookupMissRecord2D, TextureCacheInsertRecord2D, TextureCacheAddBytes2D, TextureCacheHitRecord2D) were never called; callers use the non-suffixed "same as 2D" versions. Removed from texture_tracker.hpp and texture_tracker.cpp.
- **Cleanup: remove unused version_check APIs** - `FetchLatestDebugRelease` and `DownloadDcLatestDebugToDll` had no callers (UI uses `FetchLatestDebugReleaseVersion` and `DownloadDcLatestDebugToDebugFolder`). Removed declarations and implementations. Details: utils/version_check.hpp, utils/version_check.cpp.
- **Cleanup: remove unused texture tracker total_misses** - `TextureTrackerStats.total_misses` and `g_total_misses` were written but never read by any UI or other code. Removed the field and atomic; `RecordMiss()` is now a no-op kept as a hook for future stats. Details: utils/texture_tracker.hpp, utils/texture_tracker.cpp.
- **Cleanup: remove unused texture tracker misses-per-sec EMA** - The exponential moving average of "misses per second" was computed and stored in `TextureTrackerStats.misses_per_sec_ema` but never read by any UI or other code. Removed the field, the `g_misses_per_sec_ema` and `g_last_miss_time_ns` atomics, the EMA update logic in `RecordMiss()`, and the `timing.hpp` include from texture_tracker. Details: utils/texture_tracker.hpp, utils/texture_tracker.cpp.
- **Cleanup: remove write-only additional HID hooks flag** - `g_additional_hid_hooks_installed` was only ever written via `MarkAdditionalHIDHooksInstalled` and never read. Removed the static variable and made the setter a no-op; removed unused `<atomic>` include from hid_additional_hooks.cpp.
- **Cleanup: remove unused ReShade config constant** - KEY_SHARED_PATH ("ReshadeSharedPath") in reshade_load_path.cpp was legacy and never referenced; removed.
- **Cleanup: remove unused NGX hook count getters** - GetNGXHookCount and GetTotalNGXHookCount had no callers (deprecated; UI uses g_ngx_counters directly). Removed declarations and implementations; removed unused cstdint include from ngx_hooks.hpp.
- **Cleanup: remove unused counters after DLSS-fix removal** - Six NGX counter members (d3d12/d3d11 shutdown1, evaluatefeature_c, getcapabilityparameters) and four Streamline call counters (slSetD3DDevice, slSetTag, slSetTagForFrame, slEvaluateFeature) were only read by the removed DLSS-fix getters; they are now removed from globals and hooks. Removed unused `#include <vector>` from ngx_hooks.hpp. Details: globals.hpp NGXCounters; ngx_hooks.cpp detours; streamline_hooks.cpp statics and detours; ngx_hooks.hpp.
## v0.12.358
- **DLSS-fix UI and state removed** - The "DLSS-fix" subsection in the Advanced tab (Unsupported/unfinished features) and all related code have been removed. This includes the DLSS-fix API state header, the getters that reported NGX/Streamline hook state and call counts, and the collapsing UI section. NGX and Streamline hooks themselves are unchanged. Details: removed utils/dlss_fix_api_state.hpp; GetDLSSFixNGXAPIEntries / GetDLSSFixStreamlineAPIEntries and their declarations; Advanced tab DLSS-fix block.
...
- **.UI: start with independent UI open** - Place a `.UI` file (or `.UI.*`) in the game executable directory to have the independent settings window open automatically when the game starts (ReShade path only). Details: `g_start_with_independent_ui`; ProcessAttach_CheckNoReShadeMode; continuous_monitoring applies it on first tick.
- **PresentMon: defensive null and bounds checks** - Null and bounds validation added to avoid null pointer and OOB issues: `ProviderGuidByName` rejects null `provider_name`, caps provider iteration using buffer size and `TRACE_PROVIDER_INFO`, and skips entries with invalid `ProviderNameOffset`; `TryGetEventPropertyString` and `TryGetEventPropertyU64` return false when `event_record` is null; `WorkerThread` and `CleanupThread` return immediately when `manager` is null. Details: presentmon_manager.cpp.
...
- **DLSS-fix subsection in Advanced tab** - Under "Unsupported/unfinished features", a new "DLSS-fix" collapsing section shows the state of each affected API: one line per API in the form `<API_NAME> HOOKED: true/false: <call count>`. Covers the 14 NGX and 6 Streamline APIs. Details: utils/dlss_fix_api_state.hpp (DLSSFixAPIEntry, getter declarations); GetDLSSFixNGXAPIEntries / GetDLSSFixStreamlineAPIEntries in ngx_hooks and streamline_hooks; advanced_tab.cpp DLSS-fix subsection.
